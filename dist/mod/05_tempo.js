let alpha_override = null
function set_alpha_override(v) {
  alpha_override = v
}
let i = 0
let m = 0
function collect_onsets(notes2) {
  let starts = notes2
    .map(function (n) {
  ...
  for (let i2 = 0; i2 < starts.length; i2++) {
    ...
    if (last && starts[i2].t - last.t < 0.025) {
      last.w = Math.max(last.w, starts[i2].w)
    } else {
      out.push({ t: starts[i2].t, w: starts[i2].w })
    }
  }
  return out
}
function estimate_bpm(notes2, duration) {
  let onsets = collect_onsets(notes2)
  if (onsets.length < 5) return 120
  let bin = 0.01
  let len = Math.max(64, Math.ceil(Math.min(duration, 90) / bin))
  let train = new Float32Array(len)
  for (let i2 = 0; i2 < onsets.length; i2++) {
    let idx = Math.round(onsets[i2].t / bin)
    if (idx >= 0 && idx < len) train[idx] += 0.3 + onsets[i2].w
  }
...
function fit_phase(notes2, bpm) {
  ...
  let sin_sum = 0,
    cos_sum = 0
  for (let i2 = 0; i2 < onsets.length; i2++) {
    let theta = ((onsets[i2].t % beat) / beat) * Math.PI * 2
    let w = 0.3 + onsets[i2].w
...
function track_beats(notes2, duration, bpm_hint) {
  let onsets = collect_onsets(notes2)
