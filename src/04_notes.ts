...
export interface UnionResult extends Array<Note> {
  added_by_recall?: number
}
export function union_passes(
  precision: Note[],
  recall: Note[],
  preset: Preset,
) {
  const result = precision.slice() as UnionResult
  if (!recall.length) return result

  let p_amps = precision.map(function (n: Note) {
    return n.amp
  })
  ...
  let dup_sec = (preset.dup_ms != null ? preset.dup_ms : 50) / 1000
...
export function clean(notes: Note[], preset: Preset) {
  let out = notes.filter(function (n: Note) {
    return n.dur >= preset.min_dur * 0.7 && n.amp >= preset.min_amp * 0.7
  })

  out.sort(function (a: Note, b: Note) {
    return a.pitch - b.pitch || a.start - b.start
  })

  let merge_sec = preset.merge_ms / 1000
  const merged: Note[] = []
...
export function assign_velocity(notes: Note[]) {
  if (!notes.length) return notes

  let missing = notes.filter(function (n: Note) {
    return n.velocity == null
  })
  if (!missing.length) return notes
  let amps = missing.map(function (n: Note) {
    return n.amp
  })
...
export function build(
  ...
  return {
    notes: cleaned,
    split: split,
    added_by_recall: (unioned as UnionResult).added_by_recall || 0,
  }
}
