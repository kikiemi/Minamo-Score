...
export const N_PITCH = 88

function make_fft(n: number) {
...
function midi_to_hz(m: number): number {
  return 440 * Math.pow(2, (m - 69) / 12)
}

const PITCH_SPEC: { lo: number; hi: number }[] = []
const PITCH_LAG: { lo: number; hi: number }[] = []
...
export function cfp_salience(samples: Float32Array): Salience {
...
export function cfp_scale(sal: Salience): number {
  const vals = []
